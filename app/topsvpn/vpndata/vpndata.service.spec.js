...
describe('vpndata', function() {
  ...
  var vpnMockCount = 2;
  var vpnMockCategories = ['activism', 'affiliates', 'availability', 'encryption', 'ethics', 'jurisdiction', 'leakprotection', 'logging', 'policies', 'portblocking', 'pricing', 'protocols', 'website', 'company'];
  var vpnMockCategoriesCount = 14;
  ...
  it("should get the JSON data", function() {
    ...
    expect(deserializedJson).toBeUndefined();
    $httpBackend.flush();
  ...
  it('should get all VPNs', function() {
    ...
    expect(allVpns).toBeUndefined();
    $httpBackend.flush();
    expect(allVpns.length).toEqual(vpnMockCount);
    expect(allVpns[0].id).toEqual('3Monkey');
  });
  ...
  it("should get a single VPN by id", function() {
    ...
    expect(vpn.getFeatureValue('protocols', 'openvpn')).toBeTruthy();
    expect(MrlUtil.arrayContains(vpn.getCategoryList(), 'website')).toBeTruthy();
  });
  ...
  it('should map feature ids to values', function() {
    ...
    expect(featureIdToValueMap).toBeUndefined();
    $httpBackend.flush();
  ...
  it('should get a single feature value set by id', function() {
    ...
    expect(featureIdList).toBeUndefined();
    $httpBackend.flush();
  ...
  it('should map categories to feature ids', function() {
    ...
    expect(categoryToFeatureIdMap).toBeUndefined();
    $httpBackend.flush();

    var categories = Object.keys(categoryToFeatureIdMap);
    expect(categories.length).toEqual(vpnMockCategoriesCount);
    vpnMockCategories.forEach(function(mockCategory) {
      expect(categories).toContain(mockCategory);
    });

    expect(categoryToFeatureIdMap.leakprotection.length).toEqual(3);
